		...
		"columns": [{
			...
			},
			{
				...
				"render": function(data, type, row, meta) {
					data = "";
					if(row.scoreList[0].susScore >= 90 && row.scoreList[1] == null) {
						data ='<button class="btn btn-default btn-sm add2">录入</button>';
					}
					if(row.scoreList[1] != null && row.scoreList[1].susScore >= 0) {
			...
			},
			{
				...
				"render": function(data, type, row, meta) {
					data ="";
					if(row.scoreList[1].susScore >= 80 && row.scoreList[2] == null) {
						data = '<button  class="btn btn-default btn-sm add3">录入</button>';

					}
					if(row.scoreList[2] != null && row.scoreList[2].susScore >= 0) {
